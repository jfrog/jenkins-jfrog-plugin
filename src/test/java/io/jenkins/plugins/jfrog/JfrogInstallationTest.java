...
class JfrogInstallationTest extends PipelineTestBase {
    ...
    WorkflowRun runPipeline(JenkinsRule jenkins, String name) throws Exception {
        ...
        String file = "pipeline {\n" +
                ...
                "                echo 'Building..'\n" +
                "                jf '-v'\n" +
                "                jf 'c add eco --user="+ARTIFACTORY_USERNAME+" --password="+ACCESS_TOKEN+" --url="+PLATFORM_URL+"--artifactory-url="+PLATFORM_URL+"artifactory --interactive=false --overwrite=true --'\n" +
                "                jf 'c show'\n" +
                "            }\n" +
                "        }\n" +
                "     }\n" +
